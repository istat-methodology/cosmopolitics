...
def downloadAndExtractComextMonthlyDATA(url_download,prefix_file,start_data,end_data):
    ...
    for current_month in month_iter(start_data.month, start_data.year, end_data.month, end_data.year):
        ...
        try:
            ...
            logging.error("Unexpected "+str(err)+" ; type: "+str(type(err)))
            count_error+= 1
        else:
            logging.info('File loaded: '+filenameZip)  
...
def downloadfile(url_file,filename):
    ...
    try:
        ...
        logging.info('File loaded: '+filename)  
...
def monthlyProcessing():
    ...
    logging.info('Creating table base_grafi_cpa_wo ')
...
def createOutputGraphExtraUE_Trim():
    logging.info('createOutputGraphExtraUE TRIM START')
    ...
    logging.info('tr_extra_ue TRIMESTRALI file: '+TR_EXTRA_UE_TRIMESTRALI_CSV)
    logging.info('createOutputGraph TRIM END ')
...
def checkUPMicroservices():
    ...
    try:
        ...
        logging.info(' ERRROR Refresh: ' + str(e)+"<br/>\n")
