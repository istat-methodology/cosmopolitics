        ...
        <CCardBody>
          ...
          >
            ...
            <l-circle-marker
              ...
              :fillColor="getColor((marker.export/markerMax),markerMax,markerMin)"
              @click="openModal(marker.export)"
            >
              <l-tooltip :options="{ interactive: true, permanent: false }">
                <span class="tooltip-span">{{ marker.name}}  {{ marker.export}} </span>
              </l-tooltip>
            </l-circle-marker>
...
import { LMap, LTileLayer, LControl, LTooltip, LCircleMarker } from "vue2-leaflet";
...
export default {
  ...
  methods: {
    ...
    setLegend(imin, imax){
      ...
      this.colorlegend("#Legend", qScale, "quantile", {title: "Trade Variation (%) - (Base=Nov 2019)", boxHeight: 15, boxWidth: 30});
